...
- **Lead pane เดียวที่รัน Claude** — agent panes ทางขวาเป็น `tail -f /tmp/agent-logs/<role>.log` (log viewer) ไม่ใช่ Claude process
- **spawn agent ผ่าน Agent tool** **ไม่ใช่ `tmux paste`** — รับผลกลับเป็น **return value** ตรงๆ ไม่ต้อง scrape/รอ "เสร็จแล้ว"
  ...
  - ดังนั้นใช้ **`subagent_type: "general-purpose"`** แล้ว **inject role context ใน prompt**: บอก role + working dir + ให้ agent เขียน progress ลง `/tmp/agent-logs/<role>.log` (และถ้าต้องการ ให้ agent อ่าน `.claude/agents/<role>.md` ของ agent-teams เพื่อรู้ rule ของ role นั้น)
