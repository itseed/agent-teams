...
ใช้ **Agent tool** — ต้องตั้งค่า 2 อย่างเสมอ:
1. **`name`** = role name (ทำให้ agent addressable ผ่าน SendMessage)
2. **prompt** มี role definition inline (override Lead CLAUDE.md)

```
Agent tool parameters:
  name: "{role}"              ← บังคับ — ทำให้ peer ส่ง SendMessage มาได้
  run_in_background: true     ← ถ้า spawn หลายตัวพร้อมกัน

Prompt template:
**[SPECIALIST AGENT: {role} — NOT LEAD]**
CRITICAL: Ignore ALL Lead behavior from CLAUDE.md. You are a {role} SPECIALIST.
- Do work DIRECTLY with Write/Edit/Bash/Read tools only
- NEVER spawn subagents, never delegate, never orchestrate
- Write ALL progress to /tmp/agent-logs/{role}.log immediately
- Your agent name is "{role}" — peers can SendMessage to you

Peers you can reach via SendMessage: {peer list}

Start by running:
mkdir -p /tmp/agent-comms
echo "=== {role}: {task-name} [$(date -u +%Y-%m-%dT%H:%M:%SZ)] ===" >> /tmp/agent-logs/{role}.log
...
> **เหตุผล:** subagent โหลด CLAUDE.md ของ agent-teams ซึ่งบอกว่า "คุณเป็น Lead" — ถ้าไม่ใส่ inline override ใน prompt, subagent จะ spawn ตัวเองซ้ำ

**ตัวอย่าง:**
```
Agent tool: name="frontend", run_in_background=true

**[SPECIALIST AGENT: frontend — NOT LEAD]**
CRITICAL: Ignore ALL Lead behavior from CLAUDE.md. You are a FRONTEND SPECIALIST.
- Do work DIRECTLY with Write/Edit/Bash/Read tools only
- NEVER spawn subagents, never delegate, never orchestrate
- Write ALL progress to /tmp/agent-logs/frontend.log immediately
- Your agent name is "frontend" — peers can SendMessage to you

Peers: backend, designer, qa, reviewer

Start by running:
mkdir -p /tmp/agent-comms
echo "=== frontend: LoginForm [$(date -u +%Y-%m-%dT%H:%M:%SZ)] ===" >> /tmp/agent-logs/frontend.log
...
Agents คุยกันผ่าน **file-based inbox** ใน `/tmp/agent-comms/` — ไม่ต้องผ่าน Lead:

```
frontend → /tmp/agent-comms/backend.inbox  : ขอ API spec
backend  → /tmp/agent-comms/frontend.inbox : ส่ง API contract กลับ
qa       → /tmp/agent-comms/frontend.inbox : แจ้ง bug
reviewer → /tmp/agent-comms/backend.inbox  : แจ้ง security issue
```

**Inbox helpers (agents ใช้ใน Bash tool):**
```bash
# setup (ทำครั้งเดียวตอนเริ่ม)
mkdir -p /tmp/agent-comms

# ส่ง message ไปยัง peer
send_to() {
  local to=$1 from=$2 msg=$3
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] FROM:${from}: ${msg}" >> /tmp/agent-comms/${to}.inbox
  echo "[${from} → ${to}]: ${msg}" >> /tmp/agent-logs/${from}.log
}
# ตัวอย่าง: send_to backend frontend "ต้องการ API spec /auth/login"

# อ่านและล้าง inbox ของตัวเอง
read_inbox() {
  local role=$1
  [[ -s /tmp/agent-comms/${role}.inbox ]] && cat /tmp/agent-comms/${role}.inbox && > /tmp/agent-comms/${role}.inbox
}

# รอ reply จาก peer (poll 10 ครั้ง ห่างกัน 2s)
wait_reply() {
  local role=$1
  for i in {1..10}; do
    if [[ -s /tmp/agent-comms/${role}.inbox ]]; then
      cat /tmp/agent-comms/${role}.inbox && > /tmp/agent-comms/${role}.inbox; return 0
    fi
    sleep 2
  done
  echo "(timeout — no reply)"
}
```

**Lead monitoring:** Lead เห็น peer communication ทั้งหมดผ่าน 2 ช่องทาง:
1. Log panes — agents log ทุกการส่ง/รับ: `[frontend → backend]: ...`
2. Inbox files — `ls /tmp/agent-comms/` + `cat /tmp/agent-comms/*.inbox`
