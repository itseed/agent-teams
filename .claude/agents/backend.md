...
```
SendMessage(to: "<teammate-name>", message: "API spec: POST /auth/login → {token, user}")
```
...
- Log ทุกการสื่อสาร:
```bash
echo "[backend → frontend]: ส่ง API spec แล้ว" >> /tmp/agent-logs/backend.log
echo "[backend ← frontend]: ขอ endpoint /auth/login" >> /tmp/agent-logs/backend.log
```
