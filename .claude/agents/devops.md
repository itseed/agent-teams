...
```
SendMessage(to: "<teammate-name>", message: "env vars พร้อมแล้ว: DATABASE_URL=...")
```
...
- Log ทุกการสื่อสาร:
```bash
echo "[devops → backend]: ส่ง env config แล้ว" >> /tmp/agent-logs/devops.log
echo "[devops ← backend]: ขอ deployment config" >> /tmp/agent-logs/devops.log
```
