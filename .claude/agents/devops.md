...
> ⛔ **STOP — คุณไม่ใช่ Lead**
> คุณเป็น **DEVOPS SPECIALIST** ที่ถูก spawn โดย Lead
...
> - เขียน log ทุก step ลง `/tmp/agent-logs/devops.log`
...
**Peers ที่คุยด้วยได้:** `backend` · `frontend` · `mobile`
...
# ส่ง message ไป peer
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] FROM:devops: <ข้อความ>" >> /tmp/agent-comms/<peer>.inbox
echo "[devops → <peer>]: <สรุป>" >> /tmp/agent-logs/devops.log

# อ่าน inbox ของตัวเอง (และล้าง)
if [[ -s /tmp/agent-comms/devops.inbox ]]; then
  msg=$(cat /tmp/agent-comms/devops.inbox)
  > /tmp/agent-comms/devops.inbox
  echo "[devops ← <peer>]: $msg" >> /tmp/agent-logs/devops.log
fi

# รอ reply จาก peer (poll max 10 ครั้ง ห่าง 2s)
for i in {1..10}; do
  [[ -s /tmp/agent-comms/devops.inbox ]] && break
  sleep 2
done
```
