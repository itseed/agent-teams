...
# ส่ง message ไป peer
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] FROM:reviewer: <ข้อความ>" >> /tmp/agent-comms/<peer>.inbox
echo "[reviewer → <peer>]: <สรุป>" >> /tmp/agent-logs/reviewer.log

# อ่าน inbox ของตัวเอง (และล้าง)
if [[ -s /tmp/agent-comms/reviewer.inbox ]]; then
  msg=$(cat /tmp/agent-comms/reviewer.inbox)
  > /tmp/agent-comms/reviewer.inbox
  echo "[reviewer ← <peer>]: $msg" >> /tmp/agent-logs/reviewer.log
fi

# รอ reply จาก peer (poll max 10 ครั้ง ห่าง 2s)
for i in {1..10}; do
  [[ -s /tmp/agent-comms/reviewer.inbox ]] && break
  sleep 2
done
```
