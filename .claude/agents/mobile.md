...
# ส่ง message ไป peer
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] FROM:mobile: <ข้อความ>" >> /tmp/agent-comms/<peer>.inbox
echo "[mobile → <peer>]: <สรุป>" >> /tmp/agent-logs/mobile.log

# อ่าน inbox ของตัวเอง (และล้าง)
if [[ -s /tmp/agent-comms/mobile.inbox ]]; then
  msg=$(cat /tmp/agent-comms/mobile.inbox)
  > /tmp/agent-comms/mobile.inbox
  echo "[mobile ← <peer>]: $msg" >> /tmp/agent-logs/mobile.log
fi

# รอ reply จาก peer (poll max 10 ครั้ง ห่าง 2s)
for i in {1..10}; do
  [[ -s /tmp/agent-comms/mobile.inbox ]] && break
  sleep 2
done
```
