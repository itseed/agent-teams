...
```
SendMessage(to: "<teammate-name>", message: "ต้องการ API spec สำหรับ mobile app")
```
...
- Log ทุกการสื่อสาร:
```bash
echo "[mobile → backend]: ขอ API spec" >> /tmp/agent-logs/mobile.log
echo "[mobile ← backend]: ได้รับ API spec แล้ว" >> /tmp/agent-logs/mobile.log
```
