...
## Agents in Panes
| Role     | Pane ID | Index        | Status  | Current Task         |
|----------|---------|--------------|---------|----------------------|
| frontend | %12     | dev-team:0.2 | working | implement login form |
| backend  | %13     | dev-team:0.3 | idle    | —                    |
...
### 4a. Initialize `.team-state.md` หลัง split panes เสร็จ

หลัง set `@role` / `@role_color` ของทุก pane แล้ว เขียน state file ด้วย pane IDs จริง:
